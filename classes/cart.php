...
class Cart
{
    ...
    public function add_to_cart($productID, $quantity)
    {
        ...
        if ($checkCartResult) {
            ...
            $resultImage = $this->db->select($queryImage);
            if (!$resultImage) {
                return false;
            }
            $imageData = $resultImage->fetch_assoc();

            $queryCart = "INSERT INTO tbl_cart(productID, sessionID, productName, categoryName, productPrice, quantity, productImage) 
                      VALUES('$productID', '$sessionID', '{$productData['productName']}', '{$productData['categoryName']}', '{$productData['productPrice']}', '$quantity', '{$imageData['productImage']}')";

            $resultCart = $this->db->insert($queryCart);

            return $resultCart ? true : false;
        }
    }
